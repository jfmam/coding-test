...
## 아이디어
<p>일반적인 투포인터 문제의 경우 end가 끝에 도달하면 while문을 멈추는 반면 해당문제의 경우 회전초밥 문제에 맞게 순환한다는 점을 간과하였다. start 부분이 첫번째 인덱스에 도달할 때 까지 즉 s == n이 될 떄까지 반복문을 실행해야 한다.</p> 
